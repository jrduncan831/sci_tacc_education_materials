...
def init_distributed(device):   #rank, world_size):
    # 1B. Instead, use these environment variables set by pytorch and instead of explicitly defining them.
    world_size = int(os.environ['WORLD_SIZE'])
    local_rank = int(os.environ['LOCAL_RANK'])
    #torch.cuda.set_device(local_rank)                       #
    torch.device(device)
    dist.init_process_group("gloo",                   # backend being used; nccl typically used with distributed GPU training
...
def main():
    ...
    local_rank = int(os.environ['LOCAL_RANK'])
    device = torch.device("cuda:0") if local_rank==0 else torch.device("cpu")
    ...
    model = DDP(net,
                device_ids=[local_rank] if device.type=="cuda" else None ,                  # list of gpu that model lives on 
                output_device=local_rank if device.type=="cuda" else None                # where to output model
        )
